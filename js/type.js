...
var stylefill = {
	...
	loadFile : function(url, count) {
	    ...
	    req.onreadystatechange = function() {
	      ...
	      else if (this.readyState == 4 && this.status == 000) stylefill.loadStyles(count + 1);
...
var type = {
	...
	minMaxFontSize : function (selector) {
		...
		var changeSize = function () {
			...
			while (elescount-- > 0) {
				...
				if (eleFontSize <= parseFloat(min)) ele.style.fontSize = min; 
				else if (eleFontSize >= parseFloat(max)) ele.style.fontSize = max; 
		...
		var tamer = function (text, ele, height, i, method) {
			...
			if (ele.offsetHeight < height) {
				...
				if (method == 'nonBreakingSpace') setText(ele, origHTML.replace(/\s([^\s]+)$/, '&#160;$1'));
					...
					if (method.match('Spacing')) inc = inc * -1; 
					
					if (method.match('padding')) ele.style['boxSizing'] = 'border-box';
	...
	getStyle : function (ele, style, unit) {
		...
		if (ele.currentStyle) ret = ele.currentStyle[style.replace(/-([A-z])/gi, function(a,b) {return b.toUpperCase();})];
		else if (window.getComputedStyle) ret = document.defaultView.getComputedStyle(ele,null).getPropertyValue(style);
		
		if (unit) return parseFloat(ret);
		else return ret;
